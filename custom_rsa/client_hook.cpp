...
int main(int argc, char* argv[]) {
    ...
    // Create and set the secure RSA key
    {
        RSA* secure_rsa = create_custom_secure_rsa_key(key_file);
        if (!secure_rsa) {
            std::cerr << "Failed to create secure RSA key" << std::endl;
            return 1;
        }
        ...
        if (!EVP_PKEY_assign_RSA(pkey, secure_rsa)) {
